...
#edit nilai
@app.route("/editnilai/<id_nilai>", methods=["GET", "POST"])
def editnilai(id_nilai):
    cursor = db.cursor(buffered=True)
    cursor.execute(f"select * from tb_nilai WHERE id_nilai='{id_nilai}'")
    data = cursor.fetchone()
    if request.method == "POST":
        data = request.form
        nim = data['nim']
        ...
        sql = (f"UPDATE tb_nilai SET nim='{nim}', kode_mk='{kode_mk}', absen='{absen}', tugas='{tugas}', uts='{uts}', uas='{uas}' WHERE id_nilai='{id_nilai}'")
        cursor.execute(sql) 
        db.commit()
        print(data)
...
#hapus nilai
@app.route('/hapusnilai/<id_nilai>')
def hapusnilai(id_nilai):
    cursor = db.cursor()
